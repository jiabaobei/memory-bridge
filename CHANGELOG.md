...
- **指针注入（借鉴 CCR）**：预算装不下的高置信召回不再整体丢弃，转为一行句柄 `mb:#id 片段`（正文前 40 字、指针区独立 300 字预算）——AI 觉得相关再 `membridge search "片段"` 取原文。原文始终完整冻结在库里；每条从数百字节缩到几十字节。预算内行为与 v0.25 逐字节一致；低置信召回照旧一字不进（θ_c 过滤与沉默契约原样保留）。
- **稳定前缀排序（借鉴 CacheAligner）**：注入块区块顺序固定为 工作台（恒定）→ 全文条目（预算内）→ 指针区（本轮检索的易变尾巴），稳定内容靠前以命中宿主 KV-cache 前缀缓存。
- **字节一致**：指针行不带时间戳等渲染开销；RRF 融合同分改按 node_id 定序——同库同查询恒输出逐字节相同的注入块，同轮多端/多调用前缀缓存可命中。
- **修复**：MCP `memory_search(as_context=true)` 此前漏传召回理由 `reasons`（v0.14 的标注只在 CLI 生效、MCP 主用路径丢失），统一走 `search_with_reasons` 补齐，CLI 与 MCP 行为一致。
- **测试**：新增 4 例（指针区生成与内容冻结守卫、预算充足无指针区、沉默契约与置信过滤一字未改、同库同查询两次输出逐字节相同）；全量回归与 v0.25 基线零差异（沙箱缺 rclone/计划任务的 24 例环境限制失败在改版前后完全一致）。

## [0.25.0] - 2026-09-03

安装第一步升级：从「甩文档」变成「引导接线 + 连接达标门槛」。

- **起因**：新用户第一步只收到一大段云盘指南文本，装完通道常常半截子，后面所有自动化落空；真机首接坚果云时用户要自己摸步骤。
- **交互式引导接线**：未检测到云盘客户端时，init 进入引导流——先一屏安心文案（坚果云免费版够用：一年记忆不到 1G、容量充足；配的是可随时一键作废的应用密码而非账户密码；配好后每 15 分钟全自动），再逐步：账号 → 网页生成应用密码（指路）→ 当场输入 → 脚本跑三步接线并当场报结果。检测到既有云盘客户端的老用户仍走零配置捷径，不重配。
- **连接达标门槛（带明确出口）**：坚果云主通道连接达标才继续安装；未达标可当场重试（最多 3 次）或明确选「稍后配置」（记忆仅存本机，重跑 init 补齐）——门槛不拦安装。
- **备胎顺问**：主通道达标后顺问 OneDrive 备胎（推荐可跳过），接上即登记 backup 角色；主备定位文案不变（永远一条主通道）。
- **无交互环境不变**：网页容器等无键盘环境照旧走参数化一行命令接线。
- **测试**：新增 3 例引导流用例（主通道达标登记、稍后出口不拦安装、备胎可选），全量 137/137 全绿。

## [0.24.0] - 2026-09-03

自动同步全平台：零点击承诺补齐非 Windows 缺失的两环。

- **起因**：设计承诺「init 之后零点击全自动」，但计划任务只在 Windows 注册；Linux / macOS（含网页容器）不手动跑就永不动。且自动任务只做包级发布/取回，网页端这种靠 rclone 拉文件夹的链路不在自动循环里——真机接线后仍要手动跑 `sync --netdisk`。
- **文件夹轮进自动循环**：`autosync` 先按接线状态逐家跑文件夹级双向（主通道先），再跑包级；本机云盘客户端机器没有授权段静默跳过（客户端自己维持文件夹同步），Windows 老行为不变。
- **Linux / macOS 自动任务**：init 在 Linux 注册用户 cron（`*/15`、带标记行、幂等），macOS 写 LaunchAgent plist；与 Windows 计划任务同节奏。注册失败只告警不阻塞，手动 sync 永远兜底。
- **测试**：新增 3 例（接线机器自动循环含文件夹轮、无授权段静默跳过、cron 注册幂等），全量全绿。

## [0.23.1] - 2026-09-03

坚果云首接两处真机修复：凭据写错位置、以及 WebDAV 根下「我的坚果云」一层未对齐。

- **修复一**：配置文件不存在时 `rclone config file` 首行是说明文字，v0.23.0 把说明行当成路径，凭据写进怪名文件、首次拉取找不到授权段；`rclone_config_path()` 改为跳过说明行取真实路径。
- **修复二**：坚果云 WebDAV 根下通常有一层「我的坚果云」，PC 客户端的通道文件夹实际在其下；接线时自动探测——根目录没有通道而「我的坚果云」下有时，授权段 url 自动指到那一层（配置即状态，后续同步自然同路径）。
- **修复三**：`sync --netdisk` 省略 `--dir` 时网盘轮先于目录回落执行、直接崩溃；改为先回落再跑网盘轮。
- **修复四**：接线状态文件把「远端子路径」误存成本机目录，真机首轮双向同步拼出 `membridge_jgy:/本地路径` 直接 409；改存远端子路径（本机目录仍记在 `local_dir`）。
- **测试**：新增回归用例（说明行解析、子层探测改写 url、`--netdisk` 目录回落），全量 130/130 全绿，坚果云真机首同步打通。

## [0.23.0] - 2026-09-03

口令保险库全平台覆盖（合并版）：Linux / macOS 也有与 Windows 同定位的自动同步口令托管。
本版以 PC 端推送的 v0.22.0（体检直连判定 + `--dir` 省略回落）为底合并，两项工作并存。

- **起因**：口令托管此前仅 Windows（DPAPI）。Linux / macOS 上 `save_passphrase` 直接抛错，自动同步只能靠手动传 `--passphrase`；测试套件里 5 例口令相关用例也因此长期在容器里红着。
- **文件保险库**：Linux / macOS 用 `~/.membridge/vault.key`（32 随机字节，目录 700 / 文件 600，绑定本机用户账户——与 DPAPI 的用户绑定同定位）做 SHA-256 计数流加密，零第三方依赖。密文带 `mbvault1:` 前缀，与 DPAPI 密文可区分、可共存。
- **降级一致**：密钥文件丢失或损坏视为「未托管」返回 None，不抛出——与 DPAPI 换用户/换机器时的降级路径一致，自动同步会退回通道密钥兜底。
- **兼容**：Windows 行为完全不变；`supported()` 现覆盖 Windows / Linux / macOS；`set-passphrase` 的平台提示改为中性文案；v0.22.0 的体检直连判定与 `--dir` 回落原样保留。
- **测试**：新增 `tests/test_vault.py` 6 例（跨平台存取往返、覆盖写、清除、密钥文件权限、密钥丢失降级、明文不落盘），全量全绿——此前 5 例环境限制用例全部转绿，零回归。

## [0.22.0] - 2026-09-03

体检不再骗人 + 通道目录免手敲：两处真机实测暴露的易用性缺口。

- **起因**：本机真机把通道从 OneDrive 迁到坚果云后连跑体检，撞上两件小事——
  ① 通道靠本机网盘客户端同步、完全健康，`netdisk-status` 却报「未接线」，
  看着像通道没配好，实际是体检只看 rclone 授权段、不看客户端同步范围；
  ② `sync` / `netdisk-sync` 的 `--dir` 必填，每次都要敲
  `C:/Users/user/Nutstore/1/我的坚果云/membridge` 这么长的路径。
- **体检补「桌面客户端直连」判定**：新增 `inside_drive_dir()`——通道目录落在任一本机
  云盘客户端同步目录内，体检即输出「✅ 桌面客户端直连，无需 rclone 接线」，
  `netdisk-status` 的接线状态也相应改为「未用 rclone 接线（已由本机客户端直接同步）」，
  不再把正常状态报成故障。
- **`--dir` 省略时回落已配置通道**：`sync` / `netdisk-sync` / `netdisk-status` 三个命令
  统一经 `_fill_dir_from_netdisk()` 回落——省掉每次敲长路径；既没给 `--dir` 又未配置
  通道时给可行动提示（请先 `init`）而非报错。
- **兼容性（锦上添花，不颠覆）**：显式给了 `--dir` 就直接通过、不读库、行为一字未改；
  `netdisk-connect` 的 `--dir` 仍必填（接线本就要指定目录，可能是新目录）；
  不新增状态、不碰记忆内容、不碰加密链路。
- **测试**：新增 3 例（直连判定正反、体检文案、`--dir` 回落三分支），
  3/3 通过；本机真机实测 `sync` / `netdisk-status` 省略 `--dir` 均正常，
  无通道库时提示正确。同文件既有用例中依赖 sh 假 rclone 的部分在 Windows
  下受环境限制（无 sh 执行能力）无法验证，非本次改动引入。

## [0.21.0] - 2026-09-02

旧版 rclone 兼容：官方下载通道不可达的环境也能完成三端直达。

- **起因**：网页端容器实测——rclone 官方下载站与 GitHub 二进制发布均不可达（超时），唯系统包管理器（apt）可用，但装出的是无 `bisync` 的旧版（<1.58）。
- **安装链补一环**：`install_rclone` 先试系统包管理器（apt），成功即收工，失败才回退官方包下载。
- **双向同步降级**：探测到旧版 rclone（无 bisync）自动改走「双向复制」——通道目录 append-only（差分包只增不删），先取后推的双向复制与并集同步等价、不会误删文件，输出里明标「双向复制兜底」。
- **测试**：新增旧版 rclone 仿真用例（bisync 报错 → 先取后推两次 copy + 基线标记），全量 118 例除既有 5 例环境限制外全绿，零回归。
