...
口令保险库全平台覆盖：Linux / macOS 也有与 Windows 同定位的自动同步口令托管。
...
- **兼容**：Windows 行为完全不变；`supported()` 现覆盖 Windows / Linux / macOS；`set-passphrase` 的平台提示改为中性文案。
- **测试**：新增 `tests/test_vault.py`（跨平台存取往返、覆盖写、清除、密钥文件权限 600/700、密钥丢失降级、密文前缀与明文不落盘），全量 124/124 全绿——此前 5 例环境限制用例全部转绿，零回归。
